...
> 本文件是单文件版交接说明书，**不依赖任何外部仓库或链接**。Agent 只需读取本文件 + 随附的 `build_report.py` 即可从零搭出本产品。
...
| 渲染 | 随附 `build_report.py`（**不要重写，直接使用**） | 约定写法 Markdown → 自包含 ECharts HTML |
...
用户输入(自然语言)
   ...
   → [渲染] build_report.render(md)  → 自包含 ECharts HTML
   → [前端] 打开 /report/{file}      + 首页历史列表
```

> 关键纪律：**内容（Markdown 约定写法）与呈现（渲染器）解耦**。改视觉只动 `build_report.py`；改分析逻辑只动报告撰写提示词。
...
├─ scripts/build_report.py        # 随附文件，现有渲染器，直接复用
├─ reports/
│  ├─ *.md                        # 生成的报告源（约定写法）
│  └─ output/*.html               # 渲染产物
└─ web/
...
def run(goal):
    ...
    render(md_path, html_path)                  # 5. 渲染（调用 build_report.render）
...
## 6. 渲染器契约（build_report.py 已实现，直接复用）
...
> **重要**：随本文件一并发送的 `build_report.py` 已完整实现上述契约，请直接复用，不要重写。你只需重写 `web/backend` 与 `web/frontend`，并正确调用 `build_report.render()`。
...
## 11. 给执行 Agent 的开工指令（整段复制给他）

> 请基于我发给你的两个文件（`AGENT_BRIEF.md` + `build_report.py`）从零搭一个 AI 竞品研究网站：
> 1) 用 FastAPI 做后端，提供 `POST /api/research`（接收自然语言目标）与 `GET /report/{file}`；
> 2) 后端接 DeepSeek（意图解析+报告撰写，提示词见 Brief §5）与 Tavily（实时检索，策略见 §5.2）；
> 3) 报告用固定 Markdown 约定写法（§5.3），由随附的 `build_report.py` 渲染成 ECharts HTML，**不要重写渲染器**；
> 4) 前端一个输入框 + 生成按钮 + 历史列表；
> 5) 全程遵守证据纪律：结论分事实/洞察/建议，来源可溯源，绝不编造数据。
> **先完整读完 `AGENT_BRIEF.md` 再开工。**
